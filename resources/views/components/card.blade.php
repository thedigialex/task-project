...
<a href="{{ $linkUrl }}">
    <div class="group shadow-lg p-5 rounded-md transition-transform duration-200 w-48 hover:scale-105 bg-slate-800">
        <div class="text-center mb-4">
            @isset($fa_icon)
            <i class="{{ $fa_icon }} text-4xl text-white group-hover:text-cyan-400"></i>
            @else
        ...
        <div class="text-center mb-2.5">
            <h3 class="text-lg font-semibold text-white group-hover:text-cyan-400">{{ $name }}</h3>
        </div>
        @isset ($status)
        <div class="text-center mb-2.5 text-white group-hover:text-cyan-400">
            <p>Status:
                @if ($status == 100)
